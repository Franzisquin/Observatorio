...
temp_gpkg_path = os.path.join(tempfile.gettempdir(), "temp_ro_2002.gpkg")
...
try:
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT DISTINCT nm_localidade FROM locais_votacao_2002_padronizado WHERE sg_uf='RO' ORDER BY nm_localidade;")
    print("RO distinct names in 2002 GPKG:", [r[0] for r in cursor.fetchall()])
    conn.close()
finally:
