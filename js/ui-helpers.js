...
function enhanceSelectElement(select) {
  ...
  const rebuildOptions = () => {
    ...
    options.forEach((opt, idx) => {
      ...
      // Opcao escondida no <select> some tambem da lista customizada: e assim
      // que os filtros indisponiveis no ano saem do dropdown demografico.
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    mutations.forEach(m => {
      if (m.type === 'childList') {
        ...
        if (m.target !== select) {
          // Mudou em uma <option> (disabled/hidden): a lista precisa ser refeita.
          shouldRebuild = true;
    ...
    if (shouldRebuild) {
      rebuildOptions();
      // Reconstruir descarta as classes de estado: reaplica a selecao atual.
      syncActiveText();
    }
  });
  observer.observe(select, { childList: true, attributes: true, subtree: true, attributeFilter: ['disabled', 'value', 'selected', 'hidden'] });
