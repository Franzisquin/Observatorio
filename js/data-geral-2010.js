  ...
  if (cargo !== 'senador') {
    const turno2Ufs = ufs.filter((sigla) => {
      ...
      return typeof hasGeneralSecondTurnArchive === 'function'
        ? hasGeneralSecondTurnArchive(2010, cargo, sigla)
        : true;
    });
    const turno2Payloads = (await Promise.all(
      turno2Ufs.map((sigla) => loadGeneralMajoritariaJson2010(cargo, sigla, 2).catch(() => null))
    )).filter((payload) => payload?.RESULTS);
...
async function onClickLoadData_Deputies_2010(uf, year) {
  ...
  if (!shouldReloadDeputyData) {
    ...
    try {
      ...
      // Reatribui locais de cidades que ainda nao existiam em 2010 (distritos do pai).
      if (window.EMANC) {
        await window.EMANC.ensureLoaded();
        window.EMANC.apply(2010, { resultsObjects: [results] });
      }
  ...
  try {
    ...
    // Reatribui as features dos locais reatribuidos (mesma chave dos RESULTS).
    if (window.EMANC) {
      await window.EMANC.ensureLoaded();
      window.EMANC.apply(2010, { features: baseGeo.features });
    }
